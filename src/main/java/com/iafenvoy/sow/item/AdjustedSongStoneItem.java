...
public class AdjustedSongStoneItem extends Item {
    ...
    public AdjustedSongStoneItem(SowAbilityCategory category) {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull TooltipContext context, @NotNull List<Component> tooltipComponents, @NotNull TooltipFlag tooltipFlag) {
        ...
        tooltipComponents.add(this.category.getCategory().appendColor(Component.translatable(String.format("block.%s_song", this.category.getCategory().getId().toLanguageKey()))));
    }
    ...
    public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
        ...
        if (world instanceof ServerLevel serverWorld) {
            if (!(entity instanceof Player player) || !AbilityData.get(player).isEnabled(SowAbilityCategory.ALL.get().toArray(AbilityCategory[]::new)))
                ...
                if (SongChunkManager.isSongChunk(serverWorld, this.category, pos))
