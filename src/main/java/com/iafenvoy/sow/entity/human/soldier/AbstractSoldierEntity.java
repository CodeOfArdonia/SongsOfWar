...
public abstract class AbstractSoldierEntity extends AbstractHumanEntity {
    ...
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(AbstractSoldierEntity.class, EntityDataSerializers.INT);

    protected AbstractSoldierEntity(EntityType<? extends Monster> entityType, Level world) {
        super(entityType, world);
        this.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(RandomHelper.randomOne(SOLDIER_WEAPONS).get()));
        this.setItemInHand(InteractionHand.OFF_HAND, this.getKingdom().getShield(world.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
    }
    ...
    public ResourceLocation getTextureId() {
        return ResourceLocation.tryBuild(SongsOfWar.MOD_ID, "textures/entity/human/soldier/" + this.getKingdom().getId() + "/soldier_" + this.getVariant() + ".png");
    }
}
