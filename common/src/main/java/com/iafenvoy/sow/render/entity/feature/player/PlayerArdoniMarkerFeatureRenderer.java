...
public class PlayerArdoniMarkerFeatureRenderer<T extends PlayerEntity, M extends PlayerEntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, T entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        ...
        if (data != null) {
            ...
            this.getContextModel().render(matrices, vertexConsumers.getBuffer(RenderLayer.getEntityTranslucentEmissive(data.marker())), light, OverlayTexture.DEFAULT_UV, color.getR(), color.getG(), color.getB(), color.getA());
        }
    }
}
